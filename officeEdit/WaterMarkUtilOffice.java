package com.repast.core.watermark;
...
//        Dispatch.put(picture, "Height", new Variant(90));
    ...
    public void setWaterMark(String waterMarkStr) {
         ...
         * 设置水印旋转角度、水印大小
         */
        Dispatch.put(shapeRange, "Rotation", new Variant(90));//旋转角度
        Dispatch.put(shapeRange, "LockAspectRatio", new Boolean(true));//调整大小时保持其长宽比例不变
        Dispatch.put(shapeRange, "Height", new Variant(110));//高
        Dispatch.put(shapeRange, "Width", new Variant(140));//宽
    ...
    public static void main(String[] argv) {
        ...
        try {
            if (d.initWordApp()) {
                d.openDocument("d:"+ File.separator+"bbb.docx");
                d.getActiveWindow();


                String imgPath="d:"+ File.separator+"aaa.jpg";
                System.err.println(imgPath);
//                d.setWaterPic(imgPath);//页中图片
                d.setWaterMark("沈阳SKCC");
                d.closeDocument();
            } else{
